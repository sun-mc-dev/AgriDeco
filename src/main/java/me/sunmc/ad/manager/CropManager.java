...
public final class CropManager {
    ...
    public boolean plant(Player player, String configId, Location loc) {
        ...
        var crop = new PlacedCrop(UUID.randomUUID(), configId, loc,
    ...
    private void register(PlacedCrop crop) {
        byUuid.put(crop.getUuid(), crop);
        byEntityId.put(crop.getVirtualEntityId(), crop.getUuid());
        chunkCount.merge(ChunkKey.of(crop.getLocation()), 1, Integer::sum);
    }
    ...
    private void unregister(@NotNull PlacedCrop crop) {
        ...
        chunkCount.computeIfPresent(ChunkKey.of(crop.getLocation()),
                (k, v) -> v <= 1 ? null : v - 1);
    }

    public void sendVisibleCropsTo(Player player) {
        int maxDist = plugin.getConfigManager().getMaxArmorStandDistance();
        long maxDistSq = (long) maxDist * maxDist;
        for (var crop : byUuid.values()) {
            if (!crop.getLocation().getWorld().equals(player.getWorld())) continue;
            if (crop.getLocation().distanceSquared(player.getLocation()) > maxDistSq) continue;
            plugin.getFoliaScheduler().runAt(crop.getLocation(),
